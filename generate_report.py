...
function ShowSVG(x){
  ...
  report += style + "</style></head><body>" + script
  ...
  column0 = [
      ...
      "t<sub>HD;DAT;wr</sub>", "t<sub>SU;DAT;rd</sub>", "t<sub>HD;DAT;rd</sub>",
      "t<sub>r</sub>", "t<sub>f</sub>", "t<sub>SU;STO</sub>", "t<sub>BUF</sub>"
  ]
  column1 = [
  ...
  column3 = [
      ...
      spec["t_high"], spec["t_SU_STA"], spec["t_HD_STA"], spec["t_SU_DAT"], "0",
      spec["t_SU_DAT"], "0", spec["t_rise_min"], spec["t_fall_min"],
  ...
  for i in range(len(headers[0])):
    column = headers[0][i]
    if i in [0, 1, 2, 6]:
      table += f"<th rowspan='2'>{column}</th>"
    elif i in [3, 5]:
      table += f"<th colspan='2'>{column}</th>"
    else:
      table += f"<th colspan='3'>{column}</th>"
  table += "</tr><tr>"
  ...
  for i in range(len(field)):
    ...
    table += f"<tr id='{field[i]}' onclick='ShowSVG(\"{field[i]}\")'>"
    for j in range(len(row)-1):
      column = row[j]
      if i in [0, 2, 4, 6, 12, 14, 16, 18, 20, 22, 24] and j in range(5):
        table += f"<td rowspan='2'>{column}</td>"
      else:
        table += f"<td>{column}</td>\n"

    last_column = row[-1]
    if last_column == "Fail":
      table += f"<td class='critical'>{last_column}</td>"

    elif last_column == "Pass":
      if float(row[-2]) < 5:
        table += f"<td class='warning'>{last_column}</td>"
      else:
        table += f"<td>{last_column}</td>"

    elif last_column == "N/A" and i >= 4:
      table += f"<td class='NA'>{last_column.strip()}</td>"
    elif i == 0:
      table += "<td rowspan='4'>Only for Informative</td>"

    table += "</tr>"
  report += (
      table + "</table><div><b>[1]</b> (V<sub>H</sub>-0.7V<sub>DD</sub>) / "
